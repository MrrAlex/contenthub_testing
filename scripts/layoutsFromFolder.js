...
if (args.length < 1) {
	...
	const copyFiles = () => {
		...
		var result = shell.cp(
			'-R',
			path.join(sampleSrcDir, 'components', '*'),
		...
		result = shell.cp(
			'-R',
			path.join(sampleSrcDir, 'layouts', '*'),
		...
		if (shell.test('-d', path.join(sampleSrcDir, 'elements'))) {
			...
			console.log(
				`Copy files from ${sampleSrcDir}/elements to local src`
			);
			result = shell.cp(
				'-R',
				path.join(sampleSrcDir, 'elements', '*'),
				elemDir
			);
			if (result.stderr != null) {
				copySuccessed = false;
		...
		if (fs.existsSync(sampleModule)) {
			console.log(
				`Copy files from ${sampleDir}/sample.module.ts to local src`
			);
			result = shell.cp(
				'-R',
				sampleModule,
				path.join(srcDir, 'sample.module.ts')
			);
			if (result.stderr != null) {
				copySuccessed = false;
			}
