...
if (args.length != 1) {
	...
	rl.question(
		`This script will publish content from project ${projectName} to your current tenant,  do you wish to continue? y/n: `,
		answer => {
			if (
				...
				fs.stat(projectDir, (err, stats) => {
					if (err) {
						console.log(
							`Project ${projectName} does not exist locally,  cloning ${repoUrl}`
						);
						shell.exec(
							`git clone ${repoUrl} ${_wrap(projectDir)}`,
							(code, stdout, stderr) => {
								deployContent();
							}
						);
						...
						console.log(
							`Project ${projectName} was already cloned`
						);
						deployContent();
					}
				});
			...
			const copyFiles = () => {
				...
				child_process.execFileSync(
					...
					['run', 'install-layouts-from-folder', projectDir],
					{ stdio: 'inherit' }
				);
			};
			...
			const deployContent = () => {
				...
				fs.stat(artifactsDir, (err, stats) => {
					if (err) {
						console.error(
							`Project ${projectDir} is not an installable layout as it does not contain a content-artifacts directory`
						);
					} else {
