...
shell.exec(
	'ng build --prod --vendor-chunk=false --source-map --output-path ' +
		tempDirName
);
...
if (sourceMapFile) {
	shell.exec(
		'source-map-explorer --html ' + sourceMapFile + ' > ' + sourceMapName
	);
	console.log('\nremoving ' + tempDirName + '\n');
	shell.rm('-r', tempDirName);
	console.log(
		'Successfully generated source map explorer at ' +
			sourceMapName +
			'. Open file in a browser to interact with the source map explorer.'
	);
} else {
