...
fsExtra.mkdirsSync('dist/assets/layouts/thumbnails', function (err) {
	if (err) {
		console.log(
			'cannot make directory dist/assets/layouts/thumbnails: ' + err
		);
	} else {
...
fs.writeFile(
	'dist/assets/buildDate.json',
	JSON.stringify(dateObj, null, ' '),
	function (error) {
		if (error) {
		...
		console.log(
			' successfully created buildDate.json and copied to dist/assets/buildDate.json'
		);
	}
);
...
var copyFiles = [
	...
	{
		src: 'src/wchLayouts/assets/layouts/thumbnails',
		dest: 'dist/assets/layouts/thumbnails',
	},
...
copyFiles.forEach(function (file) {
	fsExtra.copy(file.src, file.dest, function (err) {
		if (err) {
			...
			console.log(
				' successfully copied "' + file.src + '" to "' + file.dest + '"'
			);
		}
	});
});
